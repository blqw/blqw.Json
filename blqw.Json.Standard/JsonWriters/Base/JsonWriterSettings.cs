...
namespace blqw.JsonServices
{
    ...
    public sealed class JsonWriterSettings
    {
        ...
        public JsonWriterSelector Selector { get; }
        //循环引用对象缓存区
        private readonly IList _loopObject;
        private string _dateTimeFormatString;

        public JsonWriterSettings(TextWriter writer, IServiceProvider provider, JsonBuilderSettings settings)
        {
            ServiceProvider = provider ?? throw new ArgumentNullException(nameof(provider));
            Writer = writer ?? throw new ArgumentNullException(nameof(writer));
            Selector = provider.GetRequiredService<JsonWriterSelector>();
        ...
        public void WriteObject(object value) => WriteCheckLoop(value, null);
        public void WriteNull() => Selector.Get<DBNull>().Write(null, this);

        public void Write<T>(T value) => Selector.Get<T>().Write(value, this);

        internal void WriteCheckLoop(object value, IJsonWriter writer)
        {
            if (value == null || value is DBNull)
            {
                Selector.Get<DBNull>().Write(null, this);
                return;
            }
            if (writer == null)
            {
                var t = value.GetType();
                writer = Selector.Get(t.UnderlyingSystemType);
            }
            if (CheckLoopRef)
            {
        ...
        /// [
        /// </summary>
        public void BeginArray() => Writer.Write('[');

        /// <summary>
        /// ]
        /// </summary>
        public void EndArray() => Writer.Write(']');

        /// <summary>
        /// ,
        /// </summary>
        public void Common() => Writer.Write(',');

        /// <summary>
        /// {
        /// </summary>
        public void BeginObject() => Writer.Write('{');
        /// <summary>
        /// }
        /// </summary>
        public void EndObject() => Writer.Write('}');

        /// <summary>
        /// :
        /// </summary>
        public void Colon() => Writer.Write(':');
    }
}